...
public class Logger implements ItfLogger{
    ...
    private AP aps[] ;
    private static final String FILENAME = "/home/sd0104/files/log.txt";
     ...
     */
    public Logger(Variables v){
        this.v = v;
        this.rooms = new Room[v.N];
        this.ots = new OT[v.M];
    ...
    public void LogEnd(){
        ...
        output+=String.format("etMuseum Room # DT - room identification (1 .. 5) distance from outside gathering site, a random number between 15 and 30\n");
    ...
    public synchronized void APLog(int kind, int APid, int Eid , int val) {
        if(kind == v.logger.RId){
            aps[APid]=new AP();
            aps[APid].Rid=val;
            //set all to empty
            aps[APid].elems.clear();
        }
        else if (kind == v.logger.Id){
            Elem e = new Elem();
            e.Id = val;
            //e.Cv =-1;
            //e.Pos=-1;
        ...
        else if(kind == v.logger.Pos){
                aps[APid].elems.get(getElemIdx(APid,Eid)).Pos=val;
        }
        else if(kind == v.logger.Cv){
    ...
    public synchronized void OTLog(int kind, int Tid, int val) {
        if(kind == v.logger.Stat){
            ots[Tid].Stat=val;
        }
        else if(kind == v.logger.S){
            ots[Tid].S=val;
        }
        else if(kind == v.logger.MD){
            ots[Tid].MD=val;
        }
        //if called and complete update log
        TLogged();
